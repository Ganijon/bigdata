...
class WebLogParser extends Serializable {
     ...
     * @return An WebLogRecord instance wrapped in an Option.
     */
    def parseRecord(record: String): Option[WebLogRecord] = {
        val matcher = p.matcher(record)
        if (matcher.find) {
            Some(buildWebLogRecord(matcher))
        } else {
            None
        }
    }
    ...
    /**
     * Same as parseRecord, but returns a "Null Object" version of an WebLogRecord
     * rather than an Option.
     * 
     * @param record Assumed to be an Apache access log combined record.
     * @return An WebLogRecord instance. This will be a "Null Object" version of an
     * WebLogRecord if the parsing process fails. All fields in the Null Object
     * will be empty strings.
     */
    def parseRecordReturningNullObjectOnFailure(record: String): WebLogRecord = {
        val matcher = p.matcher(record)
        if (matcher.find) {
            buildWebLogRecord(matcher)
        } else {
            WebLogParser.nullObjectWebLogRecord
        }
    }
    
    private def buildWebLogRecord(matcher: Matcher) = {
        WebLogRecord(
            matcher.group(1),
            matcher.group(2),
            matcher.group(3),
...
object WebLogParser {
    ...
    val nullObjectWebLogRecord = WebLogRecord("", "", "", "", "", "", "", "", "")
